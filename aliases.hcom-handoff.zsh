# Appends a timestamped record to the repository's HCOM handoff file, creating
# the handoff directory and file with private permissions (0700/0600) the first
# time it's written. Buffers the full record into one write so concurrent
# writers can't interleave a record.
#
# @param  {string}  handoff_dir
#     Directory containing the handoff file, created with mode 700 if missing.
# @param  {string}  handoff_path
#     Path to the handoff file being appended to, created with mode 600 if missing.
# @param  {string}  ...
#     Remaining append arguments: --kind <kind> and optional --file <path>.
_hcom_handoff_append() {
	local handoff_dir="$1"
...
function hcom-handoff() {
	...
	case "$action" in
		path)
			...
			;;
		append)
			_hcom_handoff_append "$handoff_dir" "$handoff_path" "$@"
			;;
		close)
			if (( $# != 0 )); then
