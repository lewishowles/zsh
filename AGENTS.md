...
- Agent aliases live in `aliases.agents.zsh`; HCOM launchers are split across `aliases.hcom-core.zsh`, `aliases.hcom-team.zsh`, `aliases.hcom-learn.zsh`, `aliases.hcom-plan.zsh` and `aliases.hcom-session.zsh`, with shared launcher helpers (peer tags, prompt loading) in `aliases.hcom-workflow.zsh`. Shared HCOM role prompts live in `/Users/lewis/Dev/Configuration/Agents/teams/hcom/roles/` and are injected through `_hcom_launch_role`.
